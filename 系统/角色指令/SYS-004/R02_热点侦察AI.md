...
4. 在 DISCOVERY 并行阶段，必须从 `生产/当前进度.json.parallel_group.role_results` 中筛选 `role_id=R02`：
   - 恰好1条且 `task_path` 非空：该task_path才是你的唯一正式任务入口；
   - 0条：`BLOCKED_INPUT_MISSING`；
   - 多于1条、task_path重复、task文件内部role_id不是R02、或版本/依赖与parallel_group不一致：`BLOCKED_CONFLICT`。
...
- `role_id=R02`
- 当前任务 path 必须等于parallel_group.role_results中 `role_id=R02` 的唯一task_path；并行阶段不得用active_task_path兜底
